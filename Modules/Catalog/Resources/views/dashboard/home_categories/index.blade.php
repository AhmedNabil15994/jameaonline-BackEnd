...
@section('title', __('catalog::dashboard.home_categories.routes.index'))
        ...
        <div class="page-content">
            <div class="page-bar">
                <ul class="page-breadcrumb">
                    ...
                    <li>
                        <a href="#">{{__('catalog::dashboard.home_categories.routes.index')}}</a>
                    </li>
                </ul>
            </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="portlet light bordered">
                        ...
                        @permission('add_home_categories')
                        <div class="table-toolbar">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="btn-group">
                                        <a href="{{ url(route('dashboard.home_categories.create')) }}"
                        ...
                        <div class="portlet-body">
                            <table class="table table-striped table-bordered table-hover" id="dataTable">
                                ...
                                <tr>
                                    ...
                                    <th>#</th>
                                    <th>{{__('catalog::dashboard.home_categories.datatable.image')}}</th>
                                    <th>{{__('catalog::dashboard.home_categories.datatable.status')}}</th>
                                    <th>{{__('catalog::dashboard.home_categories.datatable.title')}}</th>

                                    <th>{{__('catalog::dashboard.home_categories.datatable.created_at')}}</th>
                                    <th>{{__('catalog::dashboard.home_categories.datatable.options')}}</th>
                                </tr>
                                </thead>
                            </table>
                        </div>
                        <div class="row">
                            <div class="form-group">
                                <button type="submit" id="deleteChecked" class="btn red btn-sm"
                                        onclick="deleteAllChecked('{{ url(route('dashboard.home_categories.deletes')) }}')">
        ...
        function tableGenerate(data = '') {
            ...
            var dataTable =
                $('#dataTable').DataTable({
                    ...
                    ajax: {
                        url: "{{ url(route('dashboard.home_categories.datatable')) }}",
                        type: "GET",
                        data: {
                            req: data,
                        },
...
responsivePriority:1,
                            width: '13%',
                            title: '{{__('catalog::dashboard.home_categories.datatable.options')}}',
                            className: 'dt-center',
                            orderable: false,
                            render: function (data, type, full, meta) {

                                // Edit
                                var editUrl = '{{ route("dashboard.home_categories.edit", ":id") }}';
                                editUrl = editUrl.replace(':id', data);

                                // Delete
                                var deleteUrl = '{{ route("dashboard.home_categories.destroy", ":id") }}';
                                deleteUrl = deleteUrl.replace(':id', data);

                                var deleteBtn = `
                                @permission('delete_home_categories')
                                @csrf
                                ...
                                var buttons = `
                                    @permission('edit_home_categories')
