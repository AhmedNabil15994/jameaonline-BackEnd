...
<div class="tab-pane fade in" id="vendorDeliveryTimes">
    <div class="col-md-12">
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            <tbody>
                @foreach (getDays() as $k => $day)
                        ...
                        <td>
                            <label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                <input type="checkbox" class="group-checkable"
                                    {{ $vendor->deliveryTimes()->where('day_code', $k)->value('status') == 1? 'checked': '' }}
                            ...
                            <div class="form-check form-check-inline">
                                ...
                                {{-- <span class="is_full_day">
                                    <input class="form-check-input check-time" type="radio"
                                        name="is_full_day[{{ $k }}]" id="full_time-{{ $k }}"
                                        value="1"
                                        {{ $vendor->deliveryTimes()->where('day_code', $k)->first() == null ||$vendor->deliveryTimes()->where('day_code', $k)->first()->is_full_day == 1? 'checked': '' }}
                                ...
                                <span class="is_full_day">
                                    <input class="form-check-input check-time" type="radio"
                                        name="is_full_day[{{ $k }}]" id="custom_time-{{ $k }}"
                                        value="0"
                                        {{ $vendor->deliveryTimes()->where('day_code', $k)->first() == null ||(isset($vendor->deliveryTimes()->where('day_code', $k)->first()->is_full_day) && $vendor->deliveryTimes()->where('day_code', $k)->first()->is_full_day == 0)? 'checked': '' }}
                    ...
                    @if (isset(
    $vendor->deliveryTimes()->where('day_code', $k)->first()->is_full_day,
) &&
    $vendor->deliveryTimes()->where('day_code', $k)->first()->is_full_day == 0)
                        <tr id="collapse-{{ $k }}" class="">
                            <td colspan="3" id="div-content-{{ $k }}">
                                ...
                                @foreach ($vendor->deliveryTimes()->where('day_code', $k)->first()->custom_times
    as $key => $time)
