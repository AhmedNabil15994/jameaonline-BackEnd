...
<script>
    ...
    function playSound() {
        var audio = new Audio('{{ url('uploads/media/vendor-notify.wav') }}');
        audio.play();
    }
...
<script>
    function getProductCategoriesByVendorSection(e, productId = '', flag = 'create') {
        let value = e.value;
        let url = "{{ route('vendor.products.get_product_categories_by_vendor_section') }}?section_id=" + value +
            "&product_id=" + productId + "&flag=" + flag;
        createJsTree(url);
    }

    function createJsTree(url) {
        $('#selectVendorFirstlyAlert').hide();
        $('#jstree').remove();
        $('.categoriesTabContent').append('<div id="jstree"></div>');
        $('#jstree').jstree({
            'core': {
                "check_callback": true,
                'strings': {
                    'Loading ...': "{{ __('apps::dashboard.general.loader') }}",
                },
